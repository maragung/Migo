...
STATUS: BUILT. Range ditegakkan di gerbang penerimaan frame gateway (migo-gateway/src/connection.rs, dispatch_frame): opcode 243 sampai 255 yang datang dari client dijawab dengan error UNKNOWN_OPCODE dan detail publik "reserved opcode", lalu sesi ditutup sebagai ProtocolViolation — sama fatalnya dengan pelanggaran opcode khusus server, karena nomor reserved adalah nomor yang build ini berjanji untuk tidak diketahui. Span yang ditegakkan adalah 243-255 karena section 145 mencatat dua keputusan tertulis dari kepala range reserved: STORE_PURCHASE (239) dan ENTITLEMENTS (240) diambil saat v0.16.4 dan keduanya live, lalu FED_CONVERSATION_SUBSCRIBE (241) dan FED_CONVERSATION_EVENT (242) diambil untuk tier federasi conversation section 170, sehingga 240 adalah opcode client yang sah dan 241-242 adalah opcode mesh yang sah — dari client keduanya ditolak gerbang server-auth, bukan gerbang range, karena keduanya auth Server. Test registri di migo-protocol/src/lib.rs (no_opcode_lives_in_the_never_allocated_span_of_the_reserved_range) menggagalkan build bila codegen menghasilkan variant di dalam span 243-255, dan invariant yang sama didokumentasikan di dispatcher migod (migod/src/dispatch.rs) bahwa variant di span itu tidak boleh routable. Test wire di server/crates/migo-gateway/tests/gateway.rs (a_reserved_opcode_is_refused_and_closes_the_connection) mengirim opcode 250 mentah dan menuntut penolakan plus close, satu test lain (an_allocated_opcode_at_the_reserved_head_is_not_caught_by_the_range_gate) membuktikan 240 lolos dari gerbang range dan dijawab gerbang fase, dan satu lagi (the_conversation_federation_pair_passes_the_range_gate_and_hits_the_server_auth_gate) membuktikan 241-242 lolos dari gerbang range dan dijawab gerbang server-auth dengan hint yang bukan "reserved opcode".

Range 243 sampai 255 direservasi dan WAJIB tidak dipakai sampai ada keputusan tertulis.
...
Private message tetap dapat dikirim saat node tujuan tidak terjangkau, karena pesan disimpan di node pengirim dan diteruskan ketika link kembali. Inilah alasan private message tidak memerlukan sequencer global: seq-nya memang diberikan store node pengirim, dan itu desain, bukan utang. Yang perlu dibaca dengan jujur di sini: diteruskan ketika link kembali kini berlaku untuk room maupun conversation non-room, karena keduanya punya jalur federation sendiri (FED_ROOM_EVENT dan FED_CONVERSATION_EVENT), tetapi penerima di node lain tetap hanya terjangkau bila barisnya ada di store node itu: pengiriman langsung ke sesi penerima memerlukan peta akun ke node dan replikasi baris akun, device, session, dan conversation yang hari ini masih disalin dengan tangan oleh stand-in (tools/nnode dan test dm_federation.rs), dan ingest port untuk FED_PRESENCE_DIGEST belum ada sehingga digest-nya baru divalidasi lalu dicatat. Bila semua node menunjuk satu database, penerima di node lain mendapatkannya lewat sinkronisasi section 158 saat ia berlangganan ulang; dengan store per node seperti tools/2node, jalur federasi conversation yang membawa pesan itu langsung. Kedua topologi kini jujur: yang satu tidak butuh mesh untuk pesan, yang lain tidak sampai tanpa mesh
