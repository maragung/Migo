...
Private message tetap dapat dikirim saat node tujuan tidak terjangkau, karena pesan disimpan di node pengirim dan diteruskan ketika link kembali. Inilah alasan private message tidak memerlukan sequencer global. Yang perlu dibaca dengan jujur di sini: bagian yang BUILT adalah pengirimannya, yaitu pesan bertahan dan send-nya berhasil, sedangkan bagian diteruskan ketika link kembali baru berlaku untuk room, karena FED_FORWARD hanya membawa FED_ROOM_EVENT. DM dan group chat lintas node belum memiliki jalur: pengiriman langsung ke sesi penerima memerlukan peta akun ke node, dan ingest port untuk FED_PRESENCE_DIGEST belum ada sehingga digest-nya baru divalidasi lalu dicatat. Bila semua node menunjuk satu database, penerima di node lain tetap mendapatkannya lewat sinkronisasi section 158 saat ia berlangganan ulang, tetapi itu sinkronisasi dan bukan pengiriman langsung. Menaruh store per node seperti tools/2node membuat keduanya berbeda secara nyata, dan itulah yang membuat celah ini terlihat
...
Bagian ini ada supaya dokumen tidak pernah mengklaim sesuatu yang belum dibangun. Status per 2026-09-12.
