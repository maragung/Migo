...
Blok ini memegang peran yang sama seperti blok crate di atas tetapi bagi kode di luar workspace Cargo server. Empat dari lima item yang pernah berada di sini yaitu SDK TypeScript, client web, generator beban, dan client desktop sudah berpindah ke BUILT bersama test-nya masing-masing, sehingga yang tersisa adalah satu item yang belum memiliki test yang menjalankan kodenya sendiri: berkas deployment yang sudah memiliki gate statis tetapi belum memiliki smoke test yang benar-benar menyalakan stack-nya. Menandainya BUILT akan melanggar aturan pemeliharaan di akhir section ini sedangkan meninggalkannya di BELUM ADA KODE akan membuat orang menulis ulang kode yang sudah ada, dan ia hanya boleh berpindah ke arah BUILT.
