...
STATUS: BUILT. Range ditegakkan di gerbang penerimaan frame gateway (migo-gateway/src/connection.rs, dispatch_frame): opcode 247 sampai 255 yang datang dari client dijawab dengan error UNKNOWN_OPCODE dan detail publik "reserved opcode", lalu sesi ditutup sebagai ProtocolViolation — sama fatalnya dengan pelanggaran opcode khusus server, karena nomor reserved adalah nomor yang build ini berjanji untuk tidak diketahui. Span yang ditegakkan adalah 247-255 karena section 145 mencatat tiga keputusan tertulis dari kepala range reserved: STORE_PURCHASE (239) dan ENTITLEMENTS (240) diambil saat v0.16.4 dan keduanya live, lalu FED_CONVERSATION_SUBSCRIBE (241) dan FED_CONVERSATION_EVENT (242) diambil untuk tier federasi conversation section 170, kemudian FED_ACCOUNT_QUERY (243), FED_ACCOUNT_ROWS (244), FED_CONVERSATION_QUERY (245), dan FED_CONVERSATION_ROWS (246) diambil untuk tier replikasi baris section 170, sehingga 240 adalah opcode client yang sah dan 241-246 adalah opcode mesh yang sah — dari client semuanya ditolak gerbang server-auth, bukan gerbang range, karena semuanya auth Server. Test registri di migo-protocol/src/lib.rs (no_opcode_lives_in_the_never_allocated_span_of_the_reserved_range) menggagalkan build bila codegen menghasilkan variant di dalam span 247-255, invariant yang sama disapu oleh test tabel opcode di suite fuzz migo-protocol (the_opcode_table_resolves_the_low_span_and_the_reserved_tail_exactly, yang juga menuntut 243-246 resolve dan 247-255 tidak), dan invariant yang sama didokumentasikan di dispatcher migod (migod/src/dispatch.rs) bahwa variant di span itu tidak boleh routable. Test wire di server/crates/migo-gateway/tests/gateway.rs (a_reserved_opcode_is_refused_and_closes_the_connection) mengirim opcode 250 mentah dan menuntut penolakan plus close, satu test lain (an_allocated_opcode_at_the_reserved_head_is_not_caught_by_the_range_gate) membuktikan 240 lolos dari gerbang range dan dijawab gerbang fase, satu lagi (the_conversation_federation_pair_passes_the_range_gate_and_hits_the_server_auth_gate) membuktikan 241-242 lolos dari gerbang range dan dijawab gerbang server-auth dengan hint yang bukan "reserved opcode", dan satu lagi (the_row_replication_tier_passes_the_range_gate_and_hits_the_server_auth_gate) membuktikan 243-246 lolos dari gerbang range dan dijawab gerbang server-auth dengan hint yang bukan "reserved opcode", masing-masing nomor pada sesinya sendiri karena penolakan server-auth menutup sesi.

Range 247 sampai 255 direservasi dan WAJIB tidak dipakai sampai ada keputusan tertulis.
