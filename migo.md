...
STATUS: BUILT untuk registry bit 0 sampai 21 di meta.json dan untuk logika negosiasi di gateway, yang memotong bit yang diminta client terhadap set yang diiklankan node sehingga tidak ada bit yang bisa dipakai sepihak. STATUS: BUILT untuk bit yang behavioral pada node ini, yaitu CALLS yang selalu diiklankan beserta QUIC dan TCP_TRANSPORT yang diiklankan hanya selama listenernya hidup. STATUS: SPEC untuk bit VOICE_NOTE, GROUP_CALL, FEDERATION, dan RICH_PRESENCE yang terdefinisi tetapi tidak diiklankan node ini: opcode di baliknya sebagian sudah ada di kabel, tetapi bitnya belum menjadi saklar perilaku di mana pun.

Bit yang terdefinisi tetapi belum menjadi saklar perilaku pada node ini adalah VOICE_NOTE, GROUP_CALL, FEDERATION, dan RICH_PRESENCE; setiap bit dirancang menyalakan kelompok opcode yang bersangkutan (MEDIA untuk VOICE_NOTE, CALLS dan GROUP_CALL untuk range 224 sampai 238, FEDERATION untuk range 208 sampai 223, dan RICH_PRESENCE untuk status kustom), dan opcode di baliknya sebagian sudah melayani client, tetapi perpindahan perilaku berdasarkan bitnya belum ditegakkan di satu pun dispatch.
