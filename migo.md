...
NOTIFICATION_EVENT bernomor 144, yaitu opcode server-to-client dengan DeliveryClass::Droppable, sekarang memiliki jalur server-side "Gateway::emit_notification" yang encode event sekali lalu menyiarkannya ke topik User penerima lewat hub langganan dengan coalescing key per recipient, dan round-trip diuji di "migod/tests/migod.rs"; opcode 113 sampai 117 yang akan memicunya masih SPEC sehingga tidak ada pemicu otomatis dari domain, namun jalur pemicuan manual sudah hidup dan teruji; kedua puluh sembilan opcode IDL kini menyentuh kabel, yaitu 21 opcode client-to-server yang seluruhnya terutekan dan 8 opcode server-to-client yang benar-benar disiarkan
