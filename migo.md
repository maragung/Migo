...
MIGRASI LEGACY (spec #63): akun password lama upgrade di tempat — sign in dengan password, buat root lokal, terbitkan identitas ML-DSA, daftarkan public key, dan akun yang sama mendapat jalur login kedua. Tidak ada data lama yang dihancurkan, tidak ada user_id yang berubah, dan prosesnya idempotent: retry memakai root lokal yang sama dan server mendamaikan dengan public key yang sudah ada alih-alih membuat baris ganda.
