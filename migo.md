...
STATUS: BUILT untuk mesh federation, yaitu handshake, allow-list, replay window, dan routing link di migo-federation yang diuji di test suite-nya serta transport mesh yang dihidupkan migod lewat opcode 208 sampai 221; keputusan desain ada di ADR-0005 dan section 7. STATUS: SPEC untuk topology multi-region produksi, yang membutuhkan lebih dari satu region nyata.
...
STATUS: BUILT untuk FED_DIRECTORY, join eksplisit dengan allow-list, dan home region tunggal per room di migo-federation. STATUS: SPEC untuk pengarahan client ke gateway terdekat berdasarkan latensi terukur dan untuk failover antar region, yang membutuhkan topology multi-region nyata.
...
STATUS: SPEC. migod sudah ada dan tools/2node sudah menjalankan dua node berdampingan, tetapi skenario kegagalan region di bawah ini belum pernah dijalankan karena membutuhkan topology multi-region nyata, dan bagian ini menuntut harapan yang eksplisit per skenario alih-alih smoke test dua node.
...
STATUS: BUILT untuk infrastruktur dasarnya, yaitu infra/ dengan Dockerfile.migod, Dockerfile.web, docker-compose.yml, dan gate statis make infra-check di CI. STATUS: SPEC untuk rollout bertahap per region atau per persentase session, yang membutuhkan topology multi-region dan mekanisme pengiklanan granular yang belum ada.
