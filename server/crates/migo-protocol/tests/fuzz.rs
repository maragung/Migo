...
fn the_opcode_table_resolves_the_low_span_and_the_reserved_tail_exactly() {
    // Every allocated opcode lives below 251 today (the highest is the call
    // history at 250), so the whole span 0..=300 is
    ...
    for raw in 0u32..=300 {
        ...
        let in_reserved_span = (251..=255).contains(&raw);
        assert_eq!(
            resolved.is_some(),
    ...
    for raw in 251u32..=255 {
        assert_eq!(
            ...
            None,
            "opcode {raw} is inside the never-allocated span 251-255 and must not resolve"
        )
    }
    ...
    // listing), 248-249 are allocated (the call-row replication pair), and 250
    // is allocated (the call history, the same section's latest carve-out); all
    // of them must resolve — the
    ...
    assert_eq!(Opcode::from_wire(250), Some(Opcode::CallHistory));
