...
mod tests {
    ...
    fn no_opcode_lives_in_the_never_allocated_span_of_the_reserved_range() {
        ...
        // anything in 250-255 from a client. A generated variant landing inside that
        ...
        // carve-out at 243-246, the call-listing carve-out at 247, and the
        // call-row replication pair at 248-249, the only ones ever made from the
        // reserved head) has to be re-read before a number in
        // 250-255 is given out, not after.
        for &opcode in Opcode::ALL {
            let number = opcode.to_wire();
            assert!(
                !(250..=255).contains(&number),
