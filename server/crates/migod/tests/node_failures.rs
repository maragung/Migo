...
//! file cannot close — the unused `ROOM_READ_ONLY_PARTITION` code, the absent handshake
//! timeout, no automatic directory refetch on a stale epoch, and no `RECONNECT_HINT` to
//! members of a rebalanced room — are recorded in the brief rather than papered over here.
