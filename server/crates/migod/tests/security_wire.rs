...
//!
//! * **The reserved span is terminal.** Every opcode in 241..=255 — not a
...
/// Every opcode in the never-allocated span 241-255 is refused with the public
...
async fn the_listener_still_serves_after_refusing_everything_above() {
    ...
    // answer.
    for hostile in [241u32, 240, Opcode::MessageEvent.to_wire()] {
        let (mut stream, _welcome) = handshake(addr).await;
        let frame = Frame::new(
