...
bersama 10 doc-test dan 251 test TypeScript yang semuanya hijau, lalu menutup dua hal dari
...
di `migod/tests/migod.rs` yang menggerakkan gateway sungguhan melalui FakeTransport.
Commit sebelumnya menutup cacat terbuka 8b dengan mengimplementasikan
`Dispatcher::authorize_topics` di `migo-gateway` dan `AppDispatcher` di `migod`, lengkap
dengan 3 test di `migo-gateway/tests/gateway.rs` dan 5 test di `migod/tests/migod.rs` yang
menutup keempat perilaku yang dituntut kepala suite gateway. Commit sebelumnya memperbaiki
