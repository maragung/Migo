...
contract MgoTokenTest is Test {
    ...
    function test_transfer_moves_face_value() public {
        ...
        vm.prank(payer);
        token.transfer(treasury, 150 ether);
    ...
    function test_permit_grants_allowance() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(
            uint256(keccak256("mgo-permit-key")), // any key the payer controls
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
                    token.DOMAIN_SEPARATOR(),
                    keccak256(abi.encode(token.PERMIT_TYPEHASH(), payer, treasury, 10 ether, 0, block.timestamp))
                )
            )
        );
        token.permit(payer, treasury, 10 ether, block.timestamp, v, r, s);
        assertEq(token.allowance(payer, treasury), 10 ether);
    }
    ...
    function test_reinitialize_refused() public {
        vm.expectRevert();
        token.initialize(IAccessManager(authority), admin);
    }
}
...
event PaymentReceived(address indexed token, address indexed payer, uint256 amount, string reference);
