...
| C6  | An injected frame with a forged ratchet key cannot destroy a working session                                                                                                                                                                                                                                                                                                                                                                                | `ratchet.rs::a_forged_ratchet_key_cannot_destroy_the_session`, `a_tampered_ciphertext_is_refused_and_leaves_the_session_usable`; and, for the shape C6 states but did not hold in — a forged key against a receiver that _has_ a chain, where both tests above attack one that has not yet received — the two `a_forged_frame_does_not_advance_*` tests (finding F5)                                                                                                                                                                                                        |
...
- **F4 — the mid-call joiner's first key is caller wiring.** Brief section 163's
  ...
  **Closed at the mechanism level (see C26):**
  ...
  under a wrapping key derived from the _joiner's_ pairwise session (HKDF
...
- **F5 — the receive path advanced a chain before the tag verified. CLOSED, as a code change.**
  C6 claims an injected frame cannot destroy a working session, and both tests pinning it attack a
  session that has not yet received anything — so neither ever reaches the two places where
  receiving state moved before the frame was proven. In `decrypt_in_current_chain` the chain being
  tracked was advanced in place, `gap + 1` steps, before `open_with_nonce` could fail; in
  `step_receiving_chain` the chain being left behind was advanced in place,
  `previous_chain_length − received_count` steps, before the DH step and the open. Every field of
  `RatchetHeader` is public, so neither frame needs a secret: replaying the ratchet key the receiver
  already tracks — or supplying any well-formed key of one's own — with a message number at or past
  `received_count` leaves the chain moved while the counter that indexes it stays put, so the
  genuine message at that number then derives its key from the wrong position and never opens
  again. The loss is not a delay: one frame, and that conversation is undecryptable from then on, in
  both directions, at the hand of any server that relays it. Both paths now derive on a copy and
  commit only once the open succeeds, the chain advanced past is zeroized rather than dropped, and
  the rule is stated in both modules' docs. Mirrored in `packages/crypto/src/ratchet.ts`, which
  carried the same two sites, and pinned on both sides by the two tests named under C6. Worth
  recording that C6 was stated, tested, and still false in the shape that mattered — the lesson
  being that a claim about a _working_ session is not pinned by a test on a session that has
  not yet received.
...
| Client ratchet tests           | `packages/crypto/test/ratchet.test.ts`                                               | C6, in the TypeScript build (the F5 close)          |
