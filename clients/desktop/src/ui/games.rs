//! The Games window: the catalogue the server referees, as a destination of its own.
//!
...
//! play — so the window lists the catalogue and says plainly where the play happens. The list
//! is the games crate's own fixed numbering, the same three the web client's launcher offers,
//! so the window never names a game the server cannot referee.
                ...
                ui.label(
                    RichText::new(
                        "Games start inside a conversation and play out in its thread. This \
                         ...
                         nothing to start here: the catalogue above is what the server \
                         referees, and the play happens on the web client.",
                    )
